...
LOG_MODULE_REGISTER(ff_bling_kit);

#include "../ff_bling.h"
#include "../ff_gfx.h"
#include "../ff_ui.h"
#include "../ff_util.h"
#include "ff_bling_kit.h"
 ...
 */
void ff_bling_handler_kit(bling_t *p_bling) {
  if (p_bling->x < 0) {
    p_bling->x = 0;
    p_bling->direction = 1;
  }
  ...
  for (uint8_t i = 0; i < 4; i++) {
    color_rgb_t rgb = ff_gfx_color_hsv_to_rgb(p_bling->hue, 1.0, value);
    ff_gfx_draw_line(x, 0, x, HEIGHT, rgb);
    x += (0 - p_bling->direction);
    value -= (1.0 / 4.0);
  }
  ff_gfx_push_buffer();
